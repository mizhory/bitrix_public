function bizUnitForm(optionsbizUnitForm){
    ...
    self.setSumByPsnt = function (item){
        ...
        if(self.prefix == "biz_unit"){
            self.checkBudgetSum(sumItem);
        }
    ...
    self.getMaxBudgetSum = function(idBizUnit){
        var maxSum = -1;
        if(self.budgetBizUnit && self.budgetBizUnit[idBizUnit]){
    ...
    self.checkBudgetSum = function(item){
        ...
        var warningItem = document.getElementById(self.prefix + "_warning_" + numItem);
        var warningItemText = document.getElementById(self.prefix + "_warningsum_" + numItem);

        if(sum > maxBudgetSum && maxBudgetSum != -1){
            console.log(sum + " сумма больше бюджета " + maxBudgetSum);
            warningItemText.innerHTML = self.prepareSumToInput(maxBudgetSum);
            warningItem.style.display = 'block';
        } else {
            warningItemText.innerHTML = 0;
    ...
    self.setPsntBySum = function (item){
        ...
        psntItem.value = self.prepareSumToInput(psntValue);
        self.setActiveSaveBtn(item);
    ...
    self.saveItem = function (item){
        var numItem = +item.dataset.num;
        var itemSumId = self.prefix + "_sum_" + numItem;
        var itemSum = document.getElementById(itemSumId);
        var itemId = self.prefix + "_savebi_" + numItem;

        if(self.prefix == "biz_unit"){
            self.checkBudgetSum(itemSum);
        }

        if(document.getElementById(itemId).classList.contains("ui-btn-disabled")){
            return;
        }
        ...
        var selcetBtnValue = document.getElementById(self.prefix + "_selbi_" + numItem).value;


        console.log("sumItem =" + sumItem);
        console.log("numItem = " + numItem);
        console.log("self.ietmMaxSum[numItem] = " + self.ietmMaxSum[numItem]);

        if(true/*sumItem < self.ietmMaxSum[numItem]*/){
            ...
            if(true/*nextPsnt > 0 && nextSum > 0*/){
                //self.addNewItem(numItem + 1);
                //self.updateItem(numItem + 1, nextPsnt, nextSum);
            }
        }
        self.setDisableOkButton(itemId, psntItem, sumItem, selcetBtnValue);
    };
    ...
    self.addItem = function (item){
        ...
        var itemId = self.prefix + "_addbi_" + numItem;
        ...
        self.disableAddItem(itemId);
    }

    self.setDisableOkButton = function(itemId){
        if(true/*selcetBtnValue > 0 && psntItem > 0 && sumItem > 0*/){
            document.getElementById(itemId).classList.add("ui-btn-disabled");
            document.getElementById(itemId).classList.remove("ui-btn-success");
        }
    };

    self.disableAddItem = function (itemId){
        document.getElementById(itemId).classList.add("ui-btn-disabled");
        document.getElementById(itemId).classList.remove("ui-btn-success");
    }
    ...
    self.setActiveSaveBtn = function (item){
        ...
        if(valSelect > 0 && valPsnt > 0 && valSum > 0){
        ...
        if(self.prefix == "biz_unit"){
            self.checkBudgetSum(document.getElementById(sumItemId));
        }
    ...
    self.deleteItem = function(item){
        ...
        self.enabledSuccesButton(self.prefix + "_addbi_"+(numItem-1))
    };
    ...
    self.enabledSuccesButton = function (id){
        document.getElementById(id).classList.remove('ui-btn-disabled');
        document.getElementById(id).classList.add('ui-btn-success');
    }
